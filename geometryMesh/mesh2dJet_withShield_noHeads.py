...
nameModel = "mesh2dJet_withShield_noHeads"
mouthHeight = 20
shieldLength =200
...
# mouth surface & grid
P1 = geoMod.addPoint(190, 0, 0, meshSize)
P2 = geoMod.addPoint(190, depth, 0, meshSize)
P3 = geoMod.addPoint(210, 0, 0, meshSize)
P4 = geoMod.addPoint(210, depth, 0, meshSize)
...
geoMod.mesh.setTransfiniteCurve(L1, 4)
geoMod.mesh.setTransfiniteCurve(L3, 4)
...
N = 12 					# number elements
r = 1.09 				# ratio
d = [10] 				# first element thickness
for i in range(1, N): 
...
extrude1 = geoMod.extrude([(1, L4)], -190, 0, 00, [19], recombine = True)
extrude2 = geoMod.extrude([(1, L2)], 190, 0, 00, [19], recombine = True)
...
# found via GUI
gmsh.model.addPhysicalGroup(2, [152], name = "mouth")
gmsh.model.addPhysicalGroup(2, [209, 165, 139, 195, 239, 99, 55, 29, 85, 129,
								121, 77, 37, 63, 107, 217, 173, 147, 187, 231], 
								name = "empty")
gmsh.model.addPhysicalGroup(2, [64, 42, 86, 130, 125, 102, 108, 213, 
								218, 174, 196, 240, 235, 103], name = "atmos")
